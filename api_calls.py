...
@st.cache_data(show_spinner="Getting Teams...")
def get_teams():
...
@st.cache_data(show_spinner="Getting Players...")
def get_players():
...
@st.cache_data(show_spinner="Getting Schedules...")
def get_schedule():
    r = requests.get(url = global_vars.schedule_URL, headers={'Authorization': 'Bearer ' + site_token })
    schedule = r.json()
    return schedule

@st.cache_data(show_spinner="Getting Rosters...")
def get_rosters():
